package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-semaphore/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-semaphore"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/members",s.handleList)
    s.mux.HandleFunc("POST /api/members",s.handleSet)
    s.mux.HandleFunc("DELETE /api/members/{id}",s.handleDelete)
